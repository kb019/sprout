...
impl SproutPercentage {
    ...
    pub fn data(self, sprout: &Sprout<'_>) -> SproutPoints {
...
impl Shape for Sprout<'_> {
    fn draw(&self, painter: &mut Painter) {
        let pct = self.percentage.value();
        let generated;
        let points = match self.app.get_sprout_points(pct) {
            Some(cached) => cached,
            None => {
                generated = self.generate_points(pct);
                &generated
            }
        };
...
pub fn generate_sprout_points(pct: u8) -> SproutPoints {
